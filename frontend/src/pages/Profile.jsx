...
function Profile() {
  ...
  const avgGrade = sends.length
    ? Math.round(
        sends.reduce((sum, s) => sum + s.climb_grade, 0) / sends.length,
      )
    : null;
      ...
      <div className="max-w-2xl mx-auto px-6 py-8">
        ...
        {homeGym && (
          <div className="flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 mb-6">
            ...
            <div>
              <p className="text-xs italic text-amber-500 uppercase tracking-widest">
                Home gym
              </p>
        ...
        <div className="grid grid-cols-4 gap-3 mb-8">
          {[
            ...
            {
              label: "Avg grade",
              value: avgGrade !== null ? `V${avgGrade}` : "—",
            },
          ].map((stat) => (
            <div
              key={stat.label}
        ...
        <div className="flex flex-col gap-3 mb-8">
          {sends.map((send) => (
            <div
              ...
              onClick={() =>
                navigate(
                  `/gym/${send.gym_id}/wall/${send.wall_id}/climb/${send.climb_id}`,
                )
              }
            ...
            >
              <div
                ...
                style={{
                  background: COLOUR_MAP[send.climb_colour] || "#c9a98a",
                }}
              />
              <div className="flex-1 px-4 py-3">
                <p className="font-bold italic text-amber-900 text-sm">
                  {send.climb_name}
                </p>
                <p className="text-xs italic text-amber-600 mt-1">
                  {send.wall_name} · {send.gym_name}
                </p>
              </div>
        ...
        <div className="flex flex-col gap-3 mb-8">
          {reviews.map((review) => (
            <div
              ...
              onClick={() =>
                navigate(
                  `/gym/${review.gym_id}/wall/${review.wall_id}/climb/${review.climb_id}`,
                )
              }
            ...
            >
              ...
              <p className="text-xs italic text-amber-800 mb-2">
                "{review.comment}"
              </p>
              <p className="text-xs italic text-amber-500">
                {review.wall_name} · {review.gym_name}
              </p>
            </div>
          ))}
          {reviews.length === 0 && (
