...
function Leaderboard() {
    ...
    const currentUsername = jwtDecode(token).username
    ...
    return (
        <div className="min-h-screen bg-orange-50 font-serif">
            ...
            <div className="max-w-2xl mx-auto px-6 py-8">
                ...
                <h1 className="text-3xl font-bold italic text-amber-900 mb-1">Leaderboard</h1>
                <p className="text-sm italic text-amber-600 mb-6">
                    Points based on current active climbs only — resets when the gym resets.
                </p>
                ...
                {gym && (
                    <div className="flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 mb-6">
                        ...
                        <div>
                            <p className="text-sm font-bold italic text-amber-900">{gym.name}</p>
                ...
                {myRank && (
                    <>
                        <p className="text-xs font-bold tracking-widest text-amber-700 mb-3">YOUR RANKING</p>
                        <div className="flex items-center gap-4 bg-amber-50 border-2 border-amber-900 rounded-xl px-4 py-4 mb-6">
                            <span className={`text-2xl font-bold italic ${rankColour(myRank.rank)} min-w-9 text-center`}>
                                #{myRank.rank}
                            </span>
                            <div className="flex-1">
                                <p className="font-bold italic text-amber-900">@{myRank.username}</p>
                                <span className="text-xs italic px-2 py-1 rounded-full bg-orange-100 text-amber-800 mt-1 inline-block">
                                    You
                                </span>
                            </div>
                            <div className="text-right">
                                <p className="text-xl font-bold italic text-amber-900">{myRank.points} pts</p>
                                <p className="text-xs italic text-amber-500">{myRank.send_count} sends</p>
                            </div>
                        </div>
                    </>
                )}
                ...
                {/* top climbers */}
                <p className="text-xs font-bold tracking-widest text-amber-700 mb-3">TOP CLIMBERS</p>
                <div className="flex flex-col gap-2 mb-8">
                    {rankings.map((entry) => {
                        ...
                        return (
                            ...
                            >
                                {/* rank number */}
                                <span className={`text-lg font-bold italic min-w-7 text-center ${rankColour(entry.rank)}`}>
                                    {entry.rank}
                                </span>
                                ...
                                <div className="text-right shrink-0">
                                    ...
                                    <p className="text-xs italic text-amber-500">{entry.send_count} sends</p>
                                </div>
                            </div>
                        )
                    })}
                ...
                {/* points scale */}
                <p className="text-xs font-bold tracking-widest text-amber-700 mb-3">POINTS PER GRADE</p>
                <div className="bg-amber-50 border border-amber-200 rounded-xl overflow-hidden">
                    <div className="bg-orange-100 px-4 py-2 flex justify-between">
                        <span className="text-xs font-bold text-amber-800 uppercase tracking-wide">Grade</span>
                        <span className="text-xs font-bold text-amber-800 uppercase tracking-wide">Points per send</span>
                    </div>
                    {GRADE_POINTS.map((tier, i) => (
                        ...
                        >
                            <span className="text-sm italic text-amber-900 w-24 shrink-0">{tier.label}</span>
